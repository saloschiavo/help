...
km.out=kmeans(subsample3,9,nstart=25)
km.out$tot.withinss

# TODO: VISUALIZE THIS -- Provide a plot of W(k) against k for each k between 1 and 10
# TODO: Compare this estimate with the estimate obtained in Part 1 given by the gap statistic
# Comment on the accuracy of the two estimates, explain why they are different

#ggplot(data=subsample3,aes(cluster, km.out, color='blue'))+geom_point()+theme_bw()
# TODO: SUMMARIZE
...
gap=clusGap(subsample3, kmeans, K.max=10, B=200, nstart=25, iter.max=100)
k=maxSE(gap$Tab[, "gap"], gap$Tab[,"SE.sim"], method="Tibs2001SEmax")
km.out=kmeans(subsample3,k,iter.max=100, nstart=25, algorithm=c("Hartigan-Wong"))
km.out$cluster
