...
author: 
     - "Stefanie LoSchiavo (Student ID: 011649771)"
header-includes:
   - \usepackage{bbm}
   - \usepackage{amssymb}
   - \usepackage{amsmath}
...
# Standardize data so that standard deviation is 1, and save as R object stdexpProj1
stdexpProj1 = scale(gexpProj1) #standardize data
stdexpProj2 = scale(gexp3)
View(stdexpProj2)
...
stdexpProj1df = as.data.frame(stdexpProj1)
subsample3=sample(stdexpProj1df, size=50, replace=FALSE)
gap=clusGap(subsample3, kmeans, K.max=10, B=200, nstart=25, iter.max=100)
...
plot(subsample3, col=(km.out$cluster), main="K-Means Clustering Results with K=1", xlab="", ylab="", pch=20, cex=2)

# TODO: VISUALIZE CLASSIFICATION RESULTS
# TODO: SUMMARIZE CLASSIFICATION ERRORS
# USE TABLE AND LABELS1 TO OBTAIN CLASSIFICATION ERRORS
table(subsample3)
```
...
# labels and measurements for 50 random samples
# TODO: Fix this because it isn't 50 random samples, it's still only 30
stdexpProj1df = as.data.frame(stdexpProj1)
subsample3=sample(stdexpProj1df, size=50, replace=FALSE)
gap=clusGap(subsample3, kmeans, K.max=10, B=200, nstart=25, iter.max=100)
k=maxSE(gap$Tab[, "gap"], gap$Tab[,"SE.sim"], method="Tibs2001SEmax")
cluster=kmeans(subsample3,1,iter.max=100, nstart=25, algorithm=c("Hartigan-Wong"))
...
This is a situation in which clusGap shows that it is not always accurate.

# TODO: REMOVE THESE COMMENTS
If you executed the clusGap codes correctly but got 1 as the estimated number of clusters. Please relax and move on. Since the method for `clusGap` depends on a specific null distribution, for clusGap to work accurately, as you have realized, the null distribution needs to be correct and be consistent with the data. However,  often statistical methods and hence their needed nulls distributions are violated by real data. If this violation is severe, we can say that we have applied the wrong models/methods; if this violation is not severe, we can say that the models/methods have been misspecified.

For example, if you have a Student t distribution with a large number of degrees of freedom, you known that it is close to a Gaussian distribution (which is a fact in probability and can be checked visually). So, if you use a Gaussian distribution for this Student t distribution, you will only have a slight violation of model/method assumptions. However, if the Student t distribution has a small number of degrees of freedom, then it is far from a Gaussian distribution (which is a fact in probability and can be checked visually). If you still do the same replacement, you will have a sever violation.

`clusGap` seems to be a popular method, based on the number of citations its proposing paper has. Even so, a user has to be critical when applying it to real data. This is why I put clusGap in the project.
...
set.seed(123)
stdexpProj1df = as.data.frame(stdexpProj1)
subsample3=sample(stdexpProj1df, size=250, replace=FALSE)
gap=clusGap(subsample3, kmeans, K.max=10, B=200, nstart=25, iter.max=100)
k=maxSE(gap$Tab[, "gap"], gap$Tab[,"SE.sim"], method="Tibs2001SEmax")
cluster=kmeans(subsample3,k,iter.max=100, nstart=25, algorithm=c("Hartigan-Wong"))
gap
# TODO: Report findings
# TODO: Compare findings from Part 1 & Part 2. If there are differences, explain why
# TODO: Do more genes/features necessarily gives more accurate clustering or classification results?
```
...
```{R RA-3}
set.seed(123)
stdexpProjdf1 = as.data.frame(stdexpProj1)
subsample3=sample(stdexpProj1df, size=250, replace=FALSE)
subsample3

# TODO: Apply hierarchical clustering with avg linkage, single linkage, and complete linkage
# TODO: Create dendrogram
# TODO: Find height at which cutting the dendrogram gives the same number of groups in labels1
...
labels2 <- filter(labels1, values %in% c("LUAD", "BRCA"))
